...
export const buildApp = async (options: BuildOptions = {}): Promise<FastifyInstance> => {
  ...
  const specRaw = await readFile(config.openapiSpecPath, "utf8");
  ...
  await app.register(openapiGlue, {
    specification: spec,
    // biome-ignore lint/suspicious/noExplicitAny: glue accepts undefined at runtime; types are stricter
