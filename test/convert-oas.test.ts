...
import { convertOAS } from "../src/services/convert-oas.service.js";
...
describe("convertOAS", () => {
  ...
  it("downgrades 3.1 -> 3.0 (JSON)", async () => {
    const sourceSpec = {
      ...
      components: {
        schemas: { Pet: { type: "object", properties: { nickname: { type: ["string", "null"] } } } },
      },
    };
    ...
    expect(result.headers["Content-Disposition"]).toBe('attachment; filename="openapi-3-0-3.json"');
...
`;
  ...
  it("keeps existing 3.1 patch version when no targetVersion is given", async () => {
    ...
    expect(converted.openapi).toBe("3.1.2");
  });
});
